...
Module.register("weatherforecast",{
  ...
  getParams: function() {
    ...
    if (this.config.forecastEndpoint == "forecast") {
      /*
       * Since days are in 3 hour increment we must mutliply by 8
       */
      let maxNumberOfDays = this.config.maxNumberOfDays;
      maxNumberOfDays = maxNumberOfDays > 5 ? 5 : maxNumberOfDays;
      params += "&cnt=" + maxNumberOfDays * 8;
    } else {
  ...
  processWeather: function(data) {
    ...
    this.forecast = [];
    var current_day;
    var current_day_low = 1000;
    var current_day_high = -1000;
    var dayData = {}
    var current_day_rain = 0;
    for (var i = 0, count = data.list.length; i < count; i++) {
      ...
      if(day != current_day) {
        console.log(current_day, current_day_rain);
        this.forecast.push(dayData);
        current_day_low = 1000;
        current_day_high = -1000;
        dayData = {}
        current_day_rain = 0;
        current_day = day;
      }
      this.parserDataWeather(forecast); // hack issue #1017
      if(forecast.temp.max > current_day_high) {
        current_day_high = forecast.temp.max;
      }
      if(forecast.temp.min < current_day_low) {
        current_day_low = forecast.temp.min;
      }
      if (!isNaN(forecast.rain)){
        current_day_rain += forecast.rain;
      }
      dayData = {
        day: day,
        icon: this.config.iconTable[forecast.weather[0].icon],
        maxTemp: this.roundValue(current_day_high),
        minTemp: this.roundValue(current_day_low),
        rain: this.roundValue(current_day_rain)
      }
